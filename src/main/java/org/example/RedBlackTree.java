...
public class RedBlackTree<K extends Comparable<K>> {
    ...
    public boolean add(K key, K value) {
        ...
        current = new Node<>(parent, key, value);
        if (parent == null) {
            root = current;
    ...
    public Map<K, K> inOrder(){
        List<Node<K>> nodes = new ArrayList<>();
        Map<K, K> values = new HashMap<>();

        inOrder(root, nodes);

        nodes.forEach(n -> values.put(n.key, n.value));
        return values;
    }

    private void inOrder(Node<K> node, List<Node<K>> nodes){
        if(node != null) {
    ...
    public void setValue(K key, K value){
        Node<K> node = search(root, key);
        if(node != null && node.value.compareTo(node.key) == 0){
            node.value = value;
        }
    }
    ...
    private Node<K> search(Node<K> node, K key){
        ...
        Node<K> leftResult = search(node.left, key);
        if (leftResult != null){
            return leftResult;
        }
    ...
    static class Node<K extends Comparable<K>> {
        ...
        Node(Node<K> parent, K key, K value) {
            this.parent = parent;
            this.key = key;
            this.value = value;
